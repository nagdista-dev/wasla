...
// InnerTube clients ordered from least to most restricted
const INNERTUBE_CLIENTS = [
  // TV Embedded — lowest restriction, no login needed, subtitles available
  {
  ...
  // Android — usually works without bot detection
  {
...
const INNERTUBE_API_BASE = 'https://www.youtube.com/youtubei/v1/player';

/**
 * Fetch YouTube caption tracks using multiple InnerTube client strategies.
 * Tries TV Embedded → Android → iOS → HTML scrape → youtubei.js as fallback.
 */
async function fetchCaptionTracks(videoId: string): Promise<{
  ...
  // Strategy 1: InnerTube API with multiple clients (TV first — least restricted)
  for (const ctx of INNERTUBE_CLIENTS) {
    try {
      ...
      const resp = await fetch(`${INNERTUBE_API_BASE}?key=${ctx.key}&prettyPrint=false`, {
        method: 'POST',
        headers: {
      ...
      if (Array.isArray(captionTracks) && captionTracks.length > 0) {
        console.log(`[fetchCaptionTracks] Got ${captionTracks.length} tracks via ${ctx.client.clientName}`);
        return { captionTracks, playerResponse: data };
      }
    } catch (err) {
      console.warn(`[fetchCaptionTracks] ${ctx.client.clientName} failed:`, err instanceof Error ? err.message : String(err));
    }
  }

  // Strategy 2: HTML scrape — parses ytInitialPlayerResponse embedded in the page
  try {
    console.warn(`[fetchCaptionTracks] Trying HTML scrape for ${videoId}`);
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 10000);
    const watchResp = await fetch(`https://www.youtube.com/watch?v=${videoId}&bpctr=9999999999&has_verified=1`, {
      headers: {
    ...
    if (watchResp.ok) {
      ...
      if (!html.includes('class="g-recaptcha"')) {
        ...
        const match = html.match(/ytInitialPlayerResponse\s*=\s*(\{.+?\});\s*(?:var\s|<\/script>)/s)
                   ...
                   || html.match(/ytInitialPlayerResponse\s*=\s*({[\s\S]+?});\s*if\s*\(/);
        if (match) {
          try {
            const data = JSON.parse(match[1]);
            const captionTracks = data?.captions?.playerCaptionsTracklistRenderer?.captionTracks;
            if (Array.isArray(captionTracks) && captionTracks.length > 0) {
              console.log(`[fetchCaptionTracks] Got ${captionTracks.length} tracks via HTML scrape`);
              return { captionTracks, playerResponse: data };
            }
    ...
    console.warn(`[fetchCaptionTracks] HTML scrape failed:`, err instanceof Error ? err.message : String(err));
  }

  // Strategy 3: youtubei.js as last resort (slow but thorough)
  try {
    const youtube = await Innertube.create({ lang: 'en', location: 'US', generate_session_locally: true });
    const info = await youtube.getInfo(videoId);
    const captionTracks = info.captions?.caption_tracks?.map((t: any) => ({
    ...
    if (captionTracks.length > 0) {
      console.log(`[fetchCaptionTracks] Got ${captionTracks.length} tracks via youtubei.js`);
      return { captionTracks, playerResponse: {}, youtubeiInfo: info };
    }
  } catch (err) {
    console.warn(`[fetchCaptionTracks] youtubei.js failed:`, err instanceof Error ? err.message : String(err));
  }

  return null;
}

async function fetchTranscriptFromBaseUrl(baseUrl: string): Promise<{ offset: number; duration: number; text: string }[]> {
  const resp = await fetch(baseUrl, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    },
  });
  if (!resp.ok) throw new Error(`Failed to fetch transcript: ${resp.status}`);
  ...
  while ((match = pRegex.exec(xml)) !== null) {
    ...
    if (text) {
      results.push({ offset: startMs, duration: durMs, text });
    }
  }
...
export async function fetchSubtitles(videoId: string, targetLang?: string): Promise<{
  languages: TrackInfo[];
  ...
  try {
    ...
    let captionLanguages: TrackInfo[] = [];
    let finalLang = targetLang;

    if (tracksResult) {
      const { captionTracks, youtubeiInfo } = tracksResult;
      captionLanguages = captionTracks.map((track: any) => ({
        code: track.languageCode,
        name: track.name?.simpleText || track.languageName || getLanguageName(track.languageCode),
        isAutoGenerated: track.isAutoGenerated || track.kind === 'asr',
      }));

      const langCodes = captionLanguages.map(l => l.code);
      ...
      // Path A: use youtubei.js transcript (when we went through that path)
      if (finalLang && youtubeiInfo) {
        try {
          const transcriptInfo = await youtubeiInfo.getTranscript();
          const availableLangs = transcriptInfo.languages || [];
          let targetTranscript = transcriptInfo;
          if (availableLangs.length > 0) {
            const targetTrack = captionTracks.find((t: any) => t.languageCode === finalLang);
            const needle = targetTrack?.name?.simpleText?.toLowerCase() || finalLang!.toLowerCase();
            const matched = availableLangs.find((l: string) => l.toLowerCase().includes(needle));
            if (matched && matched !== transcriptInfo.selectedLanguage) {
              targetTranscript = await transcriptInfo.selectLanguage(matched);
            }
          }
          const segments = targetTranscript.transcript?.content?.body?.initial_segments || [];
          const raw = segments
            .filter((s: any) => s.type === 'TranscriptSegment')
            .map((s: any) => ({
              offset: parseInt(s.start_ms, 10),
              duration: parseInt(s.end_ms, 10) - parseInt(s.start_ms, 10),
              text: s.snippet?.text || '',
            }))
            .filter((s: any) => s.text);
          if (raw.length > 0) {
          ...
          console.warn(`[fetchSubtitles] youtubei.js getTranscript error:`, err instanceof Error ? err.message : err);
        }
      }

      // Path B: fetch from baseUrl directly (InnerTube API returned baseUrl)
      if (finalLang && !youtubeiInfo) {
        const track = captionTracks.find((t: any) => t.languageCode === finalLang)
                   || captionTracks[0];
        if (track?.baseUrl) {
          try {
            const raw = await fetchTranscriptFromBaseUrl(track.baseUrl);
            if (raw.length > 0) {
              const subtitles = raw.map(item => ({
                start: item.offset > 100 ? item.offset / 1000 : item.offset,
                duration: item.duration > 100 ? item.duration / 1000 : item.duration,
                text: item.text,
              }));
              return { languages: captionLanguages, subtitles, selectedLanguage: finalLang };
            }
          } catch (err) {
            console.warn(`[fetchSubtitles] baseUrl fetch failed:`, err instanceof Error ? err.message : err);
          }
        }
      }
    } else {
      // No InnerTube result at all — try youtube-transcript as last resort
      finalLang = targetLang || 'en';
      console.warn(`[fetchSubtitles] No InnerTube result for ${videoId}, trying youtube-transcript`);
    }

    if (!finalLang) {
      return { languages: captionLanguages, subtitles: [], selectedLanguage: '' };
    }

    // Path C: youtube-transcript library fallback
    try {
      const raw = await YoutubeTranscript.fetchTranscript(videoId, { lang: finalLang });
      if (raw.length > 0) {
        const subtitles = raw.map(item => ({
          start: typeof item.offset === 'number' && item.offset > 100 ? item.offset / 1000 : item.offset,
          duration: typeof item.duration === 'number' && item.duration > 100 ? item.duration / 1000 : item.duration,
          text: item.text,
        }));
        return { languages: captionLanguages, subtitles, selectedLanguage: finalLang };
      }
    } catch (ytErr: any) {
      console.warn(`[fetchSubtitles] youtube-transcript error: ${ytErr?.message}`);
    }

    // Return empty list rather than null — lets the UI show "no subtitles" instead of error
    return { languages: captionLanguages, subtitles: [], selectedLanguage: finalLang };

  } catch (error: any) {
    console.error(`[fetchSubtitles] Unhandled error:`, error instanceof Error ? error.message : error);
