 ...
 * FloatingButton
 *
 * The FAB trigger stays in the normal React tree (so it respects the
 * FloatingButtonContainer visibility logic). The overlay/sheet is rendered
 * into a stable portal container that is NEVER destroyed, avoiding the
 * insertBefore/removeChild NotFoundError in React concurrent mode.
 */
const FloatingButton = memo(function FloatingButton({ onAddChannel, onAddPlaylist }: FloatingButtonProps) {
  const { isRTL, t } = useLanguage();
  const [open, setOpen] = useState(false);

  // Stable portal root — module singleton, lives for the entire app lifetime.
  const portalRoot = getStablePortalRoot('floating-button');
  ...
  // Reset overflow when unmounted while open
  useEffect(() => {
    return () => {
      document.body.style.overflow = '';
    };
  ...
  return (
    <>
      {/* FAB trigger — stays in normal React tree */}
      ...
      {/* Overlay + sheet — stable portal, never destroyed */}
      {createPortal(
        <div
