...
const FloatingButton = memo(function FloatingButton({ onAddChannel, onAddPlaylist }: FloatingButtonProps) {
  ...
  // ── Stable portal container (same fix as CustomFilterDropdown) ────────────
  const portalRoot = useRef<HTMLDivElement | null>(null);
  if (!portalRoot.current) {
    const el = document.createElement('div');
    el.setAttribute('data-floating-portal', '');
  ...
  // ── FAB trigger button — stays in normal React tree ──────────────────────
  const fabButton = (
  ...
  // ── Overlay + sheet — always mounted in stable portal container ───────────
