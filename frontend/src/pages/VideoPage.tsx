...
function VideoPage() {
  ...
  const activeSubtitleIndex = learningData?.subtitles.findIndex((cue, index, subtitles) => {
  ...
  const activeSubtitle = activeSubtitleIndex >= 0 ? learningData?.subtitles[activeSubtitleIndex] : undefined;
  ...
  useEffect(() => {
    if (activeTab !== 'learning' || !embedId || learningData) return;

    const controller = new AbortController();
    setLearningLoading(true);
    setLearningError(null);

    api.get<{ success: boolean; data?: VideoLearningData; error?: string }>(
      `/video/${encodeURIComponent(embedId)}/learning`,
      ...
      .then((res) => {
        ...
        if (res.data.success && res.data.data) {
          ...
          setLearningError(res.data.error || t('videoPage.learningUnavailable'));
        }
      })
      .catch((err) => {
        if (controller.signal.aborted || axios.isCancel(err)) return;
        setLearningError(err?.response?.data?.error || t('videoPage.learningUnavailable'));
      })
      .finally(() => {
        if (!controller.signal.aborted) setLearningLoading(false);
      });

    return () => {
      controller.abort();
    };
  }, [activeTab, embedId, learningData, t]);
  ...
  const learningPanel = (
    <div className="px-4 sm:px-0">
      {learningLoading ? (
        <div className="rounded-xl border border-gray-200/60 bg-gray-50/80 p-4 dark:border-white/10 dark:bg-white/[0.04] space-y-3">
        ...
        <section className="rounded-xl border border-gray-200/60 bg-gray-50/80 p-4 sm:p-5 dark:border-white/10 dark:bg-white/[0.04]">
          <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {learningData.languageName}{learningData.isAutoGenerated ? ` · ${t('videoPage.autoGenerated')}` : ''}
            </span>
          </div>
          {learningData.subtitles.length > 0 ? (
            <>
              ...
              <div className="max-h-[420px] space-y-3 overflow-y-auto pr-1">
                {learningData.subtitles.map((cue, index) => (
                  <div
                    key={`${cue.start}-${index}`}
